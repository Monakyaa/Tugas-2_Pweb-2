...
class Keseluruhan extends DB {
    ...
    public function getData() {
        ...
        // Query SQL untuk mengambil data dari tabel, hanya yang ruangnya 'A102'
        $stmt = $conn->prepare("SELECT pengganti_id, nama_pengawas_diganti, unit_kerja, hari_tgl_penggantian, jam, ruang, nama_pengawas_pengganti, dosen_id FROM penggantian_pengawas_ujian WHERE ruang='A102'");
...
<table class="table table-bordered" id="myTablePenggantian">
    ...
    <tbody>
        <!-- Looping melalui setiap baris data dan menampilkannya -->
...
class Keseluruhan2 extends Keseluruhan {
    ...
    function getData() {
        ...
        // Query SQL untuk mengambil data dari tabel 'laporan_kerja_lembur' untuk bulan Oktober
        $stmt = $conn->prepare("SELECT lembur_id, hari_tgl_laporan, waktu, uraian_pekerjaan, keterangan, dosen_id FROM laporan_kerja_lembur WHERE month(hari_tgl_laporan)=10");
